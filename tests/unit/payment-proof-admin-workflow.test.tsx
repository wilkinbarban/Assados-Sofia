...
describe('Comprovantes PIX admin workflow', () => {
  ...
  it('renders private preview and restore confirmation', () => {
    const mutate = leasedMutation(); render(<PaymentProofAdminPanel initialProofs={[proof]} mutate={mutate} now="2026-08-27T12:00:00Z" />)
  ...
  it('shows an accessible empty state', () => { render(<PaymentProofAdminPanel initialProofs={[]} mutate={vi.fn()} now="2026-08-27T12:00:00Z" />); expect(screen.getByRole('status')).toHaveTextContent('Nenhum comprovante nesta fila') })
  it('requires a local lease before rejection controls appear and releases after success', async () => {
    const mutate = leasedMutation({ proof: { status: 'quarantined', purge_after: '2026-09-06T12:00:00.000Z' } }); render(<PaymentProofAdminPanel initialProofs={[{ ...proof, status: 'review', purge_after: null }]} mutate={mutate} now="2026-08-27T12:00:00Z" />)
  ...
  it('allows vendedores to reject ordinary proofs', async () => { render(<PaymentProofAdminPanel initialProofs={[{ ...proof, status: 'review' }]} role="vendedor" mutate={leasedMutation()} />); fireEvent.click(screen.getByRole('button', { name: 'Reservar análise' })); expect(await screen.findByRole('button', { name: 'Rejeitar' })).toBeInTheDocument() })
})
...
it('selects one or many eligible orders and reconciles only an exact total', async () => {
  ...
  render(<PaymentProofAdminPanel initialProofs={[admitted]} initialOrders={orders} mutate={mutate} now="2026-08-27T12:00:00Z" />); fireEvent.click(screen.getByRole('button', { name: 'Reservar análise' })); await screen.findByRole('button', { name: /conciliar/i }); fireEvent.click(screen.getByRole('checkbox', { name: /pedido .*3341/i })); expect(screen.getByRole('button', { name: /conciliar/i })).toBeDisabled(); fireEvent.click(screen.getByRole('checkbox', { name: /pedido .*3342/i })); fireEvent.click(screen.getByRole('button', { name: /conciliar/i })); fireEvent.click(screen.getByRole('button', { name: 'Confirmar conciliação' })); expect(mutate).toHaveBeenCalledWith(expect.objectContaining({ operation: 'reconcile', proofId: admitted.id, orderIds: orders.map(order => order.id), leaseToken: 'a'.repeat(64) }))
})
