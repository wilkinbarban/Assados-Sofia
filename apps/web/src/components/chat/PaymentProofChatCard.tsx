...
export function PaymentProofChatCard({proofId}:{proofId:string}){
 ...
 return <div className="mt-2">
  ...
  {open&&<div role="dialog" aria-modal="true" aria-label="Comprovante PIX ampliado" className="fixed inset-0 z-[80] grid place-items-center bg-black/85 p-4" onMouseDown={e=>e.target===e.currentTarget&&setOpen(false)}>
   <div className="max-h-[92vh] max-w-4xl rounded-2xl bg-white p-3"><button type="button" onClick={()=>setOpen(false)} aria-label="Fechar prévia" className="float-right text-zinc-900">×</button>{/* eslint-disable-next-line @next/next/no-img-element -- Authenticated payment-proof preview is served by a private API route. */}
   <img src={src} alt="Comprovante PIX ampliado" className="max-h-[85vh] max-w-full object-contain"/></div>
  </div>}
 </div>
}
