-- A Mercado Pago payment may first arrive after a provider-side reversal.
-- This records that fact truthfully: no local approval event is fabricated.
alter table public.pedido_payment_events
  add column if not exists observation_provenance text;

create sequence if not exists public.pedido_payment_events_ledger_sequence;

alter table public.pedido_payment_events
  add column if not exists ledger_sequence bigint;

update public.pedido_payment_events
set ledger_sequence = nextval('public.pedido_payment_events_ledger_sequence')
where ledger_sequence is null;

alter table public.pedido_payment_events
  alter column ledger_sequence set default nextval('public.pedido_payment_events_ledger_sequence'),
  alter column ledger_sequence set not null;

create unique index if not exists pedido_payment_events_ledger_sequence_key
  on public.pedido_payment_events (ledger_sequence);

alter table public.pedido_payment_events
  drop constraint if exists pedido_payment_events_observation_provenance_check;

alter table public.pedido_payment_events
  add constraint pedido_payment_events_observation_provenance_check check (
    observation_provenance is null
    or observation_provenance in (
      'provider_terminal_without_local_approval',
      'provider_reversal_after_local_approval',
...
begin
  ...
  if p_source = 'manual' then
    ...
    if found then
      ...
      return query select v_order.id, v_delivery.result_status, true, v_order.google_event_id::text;
      return;
    end if;
  else
    select e.* into v_delivery from public.pedido_payment_events e
    ...
    if p_novo_status='reembolsado'::public.status_pagamento then
      if v_reversal.id is null then
        ...
        return query select v_order.id, 'reembolsado'::public.status_pagamento, false, v_order.google_event_id::text;
        return;
      end if;
