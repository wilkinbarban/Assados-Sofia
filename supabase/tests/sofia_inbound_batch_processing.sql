...
update public.configuracoes_sistema set valor='true' where chave='SOFIA_GLOBAL_WHATSAPP_ENABLED';
...
update public.sofia_inbound_batches set scheduled_process_at='-infinity'::timestamptz where conversa_id in ('b2000000-0000-4000-8000-000000000001','b2000000-0000-4000-8000-000000000002');

-- The clone may contain due work outside this suite. Hold it with SKIP LOCKED-visible
-- row locks rather than changing its state, so every global claim below is fixture-only.
select dblink_connect('baseline_lock',:'runtime_dblink_conninfo');
select dblink_exec('baseline_lock','begin');
select dblink_send_query('baseline_lock',$$select id from public.sofia_inbound_batches where conversa_id not in ('b2000000-0000-4000-8000-000000000001','b2000000-0000-4000-8000-000000000002') and scheduled_process_at<=now() and (status='pending' or (status='processing' and claimed_until<=now())) for update$$);
select * from dblink_get_result('baseline_lock') as locked(id uuid);
select * from dblink_get_result('baseline_lock',false) as drained(value text);
select dblink_send_query('baseline_lock',$$select batch_id from public.sofia_response_outbox where batch_id not in (select id from public.sofia_inbound_batches where conversa_id in ('b2000000-0000-4000-8000-000000000001','b2000000-0000-4000-8000-000000000002')) and (status='pending' or (status='claimed' and claimed_until<=now())) for update$$);
select * from dblink_get_result('baseline_lock') as locked(batch_id uuid);
select * from dblink_get_result('baseline_lock',false) as drained(value text);
...
select is((select count(*)::integer from claims where payload->>'conversa_id' in ('b2000000-0000-4000-8000-000000000001','b2000000-0000-4000-8000-000000000002')),2,'SKIP LOCKED gives workers distinct due batches');
select is((select count(distinct payload->>'batch_id')::integer from claims where payload->>'conversa_id' in ('b2000000-0000-4000-8000-000000000001','b2000000-0000-4000-8000-000000000002')),2,'contention never duplicates a claim');
...
select is(public.claim_sofia_inbound_batch(30),null::jsonb,'two workers leave no fixture due batch available to a third claim');
...
select is((select payload->'eligibility'->>'requires_runtime_policy_check' from claims where payload->>'conversa_id' in ('b2000000-0000-4000-8000-000000000001','b2000000-0000-4000-8000-000000000002') limit 1),'true','claim requires runtime business-hours policy verification');
select is((select payload->'eligibility'->>'eligible' from claims where payload->>'conversa_id' in ('b2000000-0000-4000-8000-000000000001','b2000000-0000-4000-8000-000000000002') limit 1),'false','claim never represents partial DB eligibility as final eligibility');
...
select is((select count(*)::integer from public.sofia_inbound_batches where conversa_id='b2000000-0000-4000-8000-000000000001' and status='pending' and scheduled_process_at<=now()),0,'terminal cancellation is never reclaimed');

update public.sofia_inbound_batches set claimed_until=now()-interval '1 second',scheduled_process_at='-infinity'::timestamptz where conversa_id='b2000000-0000-4000-8000-000000000002';
...
select is((select count(*)::integer from public.mensagens where conversa_id='b2000000-0000-4000-8000-000000000002' and external_id like 'sofia-batch:%'),1,'completion persists exactly one IA message');
select is((select count(*)::integer from public.sofia_response_outbox o join public.sofia_inbound_batches b on b.id=o.batch_id where b.conversa_id='b2000000-0000-4000-8000-000000000002'),1,'completion persists exactly one response intent');
...
select is(public.claim_sofia_inbound_batch(30),null::jsonb,'expired fixture lease with a durable intent is finalized, not regenerated');
...
select dblink_exec('baseline_lock','commit');
select dblink_disconnect('baseline_lock');
